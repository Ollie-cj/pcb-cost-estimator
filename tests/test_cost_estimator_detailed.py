...
class TestCostEstimatorPriceRanges:
    ...
    def test_resistor_price_range(self, estimator):
        ...
        parse_result = BomParseResult(items=items)

        cost_estimate = estimator.estimate_bom_cost(parse_result)

        # Resistors should typically be $0.001 - $0.02 for standard values
        assert cost_estimate.total_component_cost_typical >= 0.001
        assert cost_estimate.total_component_cost_typical <= 5.0
        ...
        assert comp_cost.unit_cost_high <= 0.1
    ...
    def test_assembly_cost_calculation(self, estimator):
        ...
        small_result = BomParseResult(items=small_items)
        ...
        large_result = BomParseResult(items=large_items)
    ...
    def test_package_complexity_affects_assembly_cost(self, estimator):
        ...
        simple_result = BomParseResult(items=simple_items)
        ...
        complex_result = BomParseResult(items=complex_items)
...
class TestCostEstimatorWithRealBoms:
    ...
    def test_arduino_shield_cost_estimate(self, parser, estimator):
        ...
        # Arduino shield component cost should be in a reasonable range
        assert 0.50 <= cost_estimate.total_component_cost_typical <= 100.0
...
class TestCostEstimatorEdgeCases:
    ...
    def test_empty_bom(self, estimator):
        ...
        parse_result = BomParseResult(items=[])
