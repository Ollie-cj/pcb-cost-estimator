...
class TestEndToEndPipeline:
    ...
    def test_complete_pipeline_arduino_shield(self, parser, estimator):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert 'component_costs' in json_data or 'itemized_components' in json_data
            ...
            assert 'Cost Estimate' in md_content or 'Total Cost' in md_content or '#' in md_content
    ...
    def test_complete_pipeline_iot_board(self, parser, estimator):
        ...
        reporter = CostReportGenerator(cost_estimate)
        # generate_cli_table writes to console; just verify it doesn't raise
...
R4,1,Good component"""
    ...
    def test_pipeline_category_breakdown(self, parser, estimator):
        ...
        for comp_cost in cost_estimate.component_costs:
            category = comp_cost.category
            if isinstance(category, str):
                cat_str = category
            else:
                cat_str = category.value
            if cat_str not in category_totals:
                category_totals[cat_str] = 0.0
            category_totals[cat_str] += comp_cost.total_cost_typical

        # Should have multiple categories
        assert len(category_totals) >= 3

        # All categories should have non-negative costs
...
C1,1,Capacitor DNI"""
        ...
        assert len(cost_estimate.component_costs) == 0 or cost_estimate.total_cost_per_board_typical >= 0
...
class TestReportFormatValidation:
    ...
    def test_json_report_valid(self, sample_estimate):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cost_per_board = data['executive_summary']['cost_per_board']
            assert isinstance(cost_per_board['typical'], (int, float))
